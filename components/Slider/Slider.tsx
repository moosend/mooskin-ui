...
export const ABSlider: React.StatelessComponent<IABSliderProps> = (props) => {
    ...
    const onChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        const value =
            parseInt(e.target.value, 10) / 2 < min ? min : parseInt(e.target.value, 10) / 2 > max ? max : parseInt(e.target.value, 10) / 2;
        props.onChange && props.onChange(e, { value, dataLabel: props.dataLabel });
    };
    ...
    return (
        <div className={`${styles.abContainer} ${props.className}`} style={props.style}>
            <div className={`${styles.section} ${styles.sectionA}`} style={{ left: 0, right: `${100 - percentage}%` }}>
                <span className={styles.font}>A</span>
                <span className={styles.percentText}>
                    {percentage}% ({props.count === 0 ? 0 : percentageAB})
                </span>
            </div>
            <div className={`${styles.section} ${styles.sectionB}`} style={{ left: `${percentage}%`, right: `${100 - 2 * percentage}%` }}>
                <span className={styles.font}>B</span>
                <span className={styles.percentText}>
                    {percentage}% ({props.count === 1 || props.count === 0 ? 0 : percentageAB})
                </span>
            </div>
            <div className={`${styles.section} ${styles.sectionW}`} style={{ left: `${2 * percentage}%`, right: 0 }}>
